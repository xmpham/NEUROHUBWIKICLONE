...
```bash
singularity run SING_IMAGE.sif datafile.nii.gz  # Executes the set of commands defined under 'run' for the image 'SING\_IMAGE' on the input 'datafile.nii.gz'
singularity exec SING_IMAGE.sif ls -lth /  	# Uses 'SING\_IMAGE.sif' to execute the command, 'ls -lth /'
```
...
module load singularity/3.6
singularity shell **--overlay imaging/neurohub\_ukbb\_participants.squashfs** example_singularity.sif
